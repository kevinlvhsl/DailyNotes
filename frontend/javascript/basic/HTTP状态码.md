...
301 Moved Permanently 表示客户请求的文档在其他地方，新的URL在Location头中给出，浏览器应该自动地访问新的URL。永久重定向
302 Found 临时性重定向，表示请求的资源已经被分配了新的URI，希望用户本次使用新的URI访问
303 See Other 临时性重定向，让用户以get方式请求新的URI。 
...
403   （禁止） 服务器拒绝请求。并不想给出理由。
404   （未找到） 服务器找不到请求的网页。也可以在服务器拒绝并不想给出理由时使用。
405   （方法禁用） 禁用请求中指定的方法。 
